...
use App\Mail\CotizacionEtapa6Respuesta;
use App\Mail\CotizacionEtapa7Respuesta;
use App\Mail\CotizacionEtapa8Respuesta;
use App\Mail\CotizacionEtapa9Respuesta;
    ...
    public function create2($id)
    {
        ...
        switch ($estatus) {
            ...
            case '4':
                ...
                if(session()->get('rol')!=3){
                    return redirect()->route('home');
                }elseif(session()->get('rol')==3 && $cotizacion->empresa_id == session()->get('id_empresa')){

                $respuesta = RespuestaCotizacion::where('cotizacion_id', $id)->first();
                ...
                return view('cotizaciones.create.etapa5', compact('respuesta','horarios','contador', 'cotizacion'));
            }
                break;
            case '5':
                ...
                if(session()->get('rol')==3){
                    ...
                    $anuncio = Anuncio::where('cotizacion_id', $id)->first();
                    $extensionAudio = $anuncio->archivo_audio;
                    $extensionAudio = explode('.', $extensionAudio);
                    $extensionAudio = end($extensionAudio);
                    $extensionAudio = strtolower($extensionAudio);
                    $extensionTexto = $anuncio->archivo_texto;
                    $extensionTexto = explode('.', $extensionTexto);
                    $extensionTexto = end($extensionTexto);
                    $extensionTexto = strtolower($extensionTexto);
                

                return view('cotizaciones.create.etapa6', compact('cotizacion', 'anuncio', 'extensionAudio', 'extensionTexto'));
                }
                break;
            case '6':
                    if(session()->get('rol')!=3){
        ...
        switch ($estatus) {
            ...
            case '3':
                ...
                if($request->seleccioneOpción == '1'){
                        ...
                        $respuesta = RespuestaCotizacion::where('cotizacion_id', $cotizacion->id_cotizacion)->first();
                        //transformar $request->hora en string
                        $hora = $request->hora;
                        $hora = implode(',', $hora);
                        $respuesta->horarios = $hora;
                        ...
                        $cotizacion->fecha_inicio = $respuesta->fecha_inicio;
                        $cotizacion->fecha_fin = $respuesta->fecha_fin;
                        $cotizacion->cantidad = $respuesta->cantidad + $respuesta->frases_extras;
                        
                        $anuncio = new Anuncio();
                        $anuncio->cotizacion_id = $cotizacion->id_cotizacion;
                        //id_anuncio es la fecha y hora actual concatenada
                        $anuncio->id_anuncio = date('mdYhis', time());
                        
                        $anuncio->estatus_anuncio_id = 1;
                        $anuncio->save();
                        $cotizacion->save();
                        $respuesta->save();
            ...
            case '4':
                ...
                if($request->seleccioneOpción == '1'){
                    ...
                    // $request->validate([
                    //     'archivoDeAudio' => 'required_without:archivoDeTexto|mimes:mp3,wav',
                    //     'archivoDeTexto' => 'required_without:archivoDeAudio|mimes:doc,pdf,docx,txt',
                    // ]);
                    ...
                    //if request has file
                    if($request->hasFile('archivoDeAudio')){
                        if($request->file('archivoDeAudio')){
                            $extension =  $request->file('archivoDeAudio')->extension();
                            $anuncio->archivo_audio = $request->archivoDeAudio->storeAs('audiosClientes', $cotizacion->id_cotizacion.'.'.$extension, 'public');
                        }
                        
                    }

                    if($request->hasFile('archivoDeTexto')){
                        if($request->file('archivoDeTexto')){
                            $extension =  $request->file('archivoDeTexto')->extension();
                            $anuncio->archivo_texto = $request->archivoDeTexto->storeAs('textos', $cotizacion->id_cotizacion.'.'.$extension, 'public');
                        }
                    }

                    $anuncio->estatus_anuncio_id = 2;
                    $anuncio->save();
                    $cotizacion->estatus_id = 5;
                    $cotizacion->save();
                    return redirect()->route('cotizaciones.listado')->with('success','Se han Aceptado Correctamente los Archivos');
            ...
            case '5':
                    ...
                    $request->validate([
                        'archivoDeAudio' => 'required|mimes:mp3,wav'
                    ]);

                    $respuesta = RespuestaCotizacion::where('cotizacion_id', $cotizacion->id_cotizacion)->first();
                    $anuncio = Anuncio::where('cotizacion_id',$cotizacion->id_cotizacion)->first();
                    $anuncio->estatus_anuncio_id = 3;
                    $anuncio->save();
                    $cotizacion->estatus_id = 6;
                    $cotizacion->save();
                    $extension =  $request->file('archivoDeAudio')->extension();
                  
                    $respuesta->audio_confirmacion = $request->archivoDeAudio->storeAs('audiosMuestra', $anuncio->id_anuncio.'.'.$extension, 'public');
